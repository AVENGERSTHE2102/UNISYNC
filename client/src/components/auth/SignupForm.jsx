...
function SignupForm() {
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      setNotice('User created successfully via Firebase! Redirecting to setup portal login space...');
      setTimeout(() => navigate('/login'), 2000);
    } catch (error) {
      console.error('Firebase Registration Error:', error);
      setNotice(error.message || 'Registration failure running Cloud Auth pipeline.');
    } finally {
