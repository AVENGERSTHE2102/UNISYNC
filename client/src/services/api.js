  ...
  try {
    ...
    if (!response.ok) {
      ...
      if (response.status === 504 || response.status === 502) {
        throw new Error(`Server connection failed (${response.status}). Please make sure your backend server is running on port 3000.`);
      }
    ...
    if (error.message.includes('Failed to fetch') || error.message.includes('NetworkError')) {
      throw new Error('Could not connect to the backend server. Please verify that the Express server is running on port 3000.');
    }
    throw error;
  }
}
